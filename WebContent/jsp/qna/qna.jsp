		...
		<!-- 검색창 -->
				...
				<div class="qna_board_qna_title">
           			<form method='post' action="/bitcomu/qna/qnaWrite.do" name="crForm1">
              			<input type="text" class="qna_text_title" name="qnaTitle" id="qnaBTitle" "style="resize: none" placeholder="Title" value="${b.qnaTitle}" />
              			<input type="checkbox" id="qna_check_2" name="qnaEnabled" value="N"><i class="fa fa-lock" style="font-size: 2em" aria-hidden="true"></i>
			  			<input type="text" class="qna_text_content" name="qnaContent"  id="qnaBContent" style="resize: none" placeholder="Content" value="${b.qnaContent}" /> 
...
// 				         					console.log(${sessionScope.user.userGrade});
					...
					html +=`
								<form method='post' action="/bitcomu/qna/qnaCommentWrite.do" name="crForm3">
				                    ...
				                    <input type="text" class="qna_text_content_2" name="cmtContent" id="qnaCmtContent" style="resize: none" placeholder="Comment"/>
...
// 		console.log('세션의유저',${user.userNo});
	...
	function chkNullQna() {
		...
		if(crForm1.qnaTitle.value.length > 33 || crForm1.qnaContent.value.length > 666)
		{
			alert("제목 33자, 내용 666자 이내로 입력해주세요.");
			return false;
		}
	...
	function chkNullQnaUpdate(qnaNo) {
		...
		for (let i = 0; i < crForm2.length; i++) {
			if (crForm2[i].qnaNo.value == qnaNo) {
				...
				if(crFomVal.qnaTitle.value.length > 33 || crFomVal.qnaContent.value.length > 666)
				{
					alert("제목 33자, 내용 666자 이내로 입력해주세요.");
					return false;
				}
	...
	function chkNullQnaCmt(qnaNo) {
		...
		for (let i = 0; i < crForm3.length; i++) {
			if (crForm3[i].boardPostNo.value == qnaNo) {
				...
				if(crFomVal2.cmtContent.value.length > 100)
				{
	...
	function chkNullQnaCmtUpdate(cmtNo) {
		...
		for (let j = 0; j < crForm4.length; j++) {
			if (crForm4[j].cmtNo.value == cmtNo) {
				...
				if(crFomVal3.cmtContent.value.length > 100)
				{
					alert("100자 이내로 입력해주세요.");
					return false;
				}
